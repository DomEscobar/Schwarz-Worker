---
name: szw-phase
description: Fused phase flow — plan then execute then verify for one phase (with retry loop until automated verification passes)
...
---
<objective>
Run the full phase flow for phase N in one command: plan phase N, execute phase N (with internal retry loop until phase verification passes or max retries), then run verify-work for phase N unless --no-verify-work is set. Uses only SchwarzWorker workflows and agents.
</objective>
...
4. Unless --no-verify-work, run verify-work for N.
5. Report result and suggest next step (/szw-phase N+1 or /szw-audit-milestone).
</process>
