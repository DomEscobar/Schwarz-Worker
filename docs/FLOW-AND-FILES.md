...
| `/szw-new-project` | new-project | **W** PROJECT, REQUIREMENTS, ROADMAP, STATE, config.json; **W** research/* |
...
| `/szw-phase N` | phase-flow | Runs discuss → plan → execute → verify; touches all phase files for N |
...
| `/szw-pause-work` | pause-work | **R** ROADMAP, current phase dir; **W** STATE |
| `/szw-resume-work` | resume-work | **R** STATE |
...
| Step | Read | Write |
|------|------|--------|
| 1 | — | Create `.planning/`, `phases/`, `research/`, `milestones/`, `quick/`, `todos/pending/` |
...
| 6 | templates/project, state | `.planning/PROJECT.md`, `.planning/STATE.md`, `.planning/config.json` |

**References/templates:** `references/planning-config.md`, `templates/project.md`, `requirements.md`, `roadmap.md`, `state.md`.
...
| 2–3 | ROADMAP (phase goal) | — |
| 4 | — | `.planning/phases/{phase}/{phase}-CONTEXT.md` (e.g. 01-CONTEXT.md) |
| (optional) | — | `.planning/STATE.md` (decisions) |
...
| Step | Read | Write |
|------|------|--------|
| 1 | `.planning/ROADMAP.md`, `.planning/phases/{phase}/{phase}-CONTEXT.md`, `.planning/REQUIREMENTS.md` | — |
| 2 | CONTEXT, goal | `.planning/phases/{phase}/{phase}-RESEARCH.md` (if research enabled) |
| 3 | ROADMAP, CONTEXT, RESEARCH, REQUIREMENTS | `.planning/phases/{phase}/{phase}-01-PLAN.md`, `{phase}-02-PLAN.md`, ... |
| 4 | PLAN(s), goal, must-haves | — (plan-checker reads only; planner may overwrite PLANs on revise) |

**References/templates:** `references/verification-patterns.md`, `templates/plan.md`.
...
| Step | Read | Write |
|------|------|--------|
| 1 | `.planning/phases/{phase}/*-PLAN.md` | — |
| 2 | Each PLAN, codebase, AGENTS.md | `.planning/phases/{phase}/{phase}-01-SUMMARY.md`, `{phase}-02-SUMMARY.md`, ...; git commits |
| 3 | ROADMAP, PLAN(s) must_haves, codebase | `.planning/phases/{phase}/{phase}-VERIFICATION.md` |
...
Runs **discuss-phase** → **plan-phase** → **execute-phase** (loop until verification passed or max retries) → **verify-work**.

So for phase N it touches the **union** of files from 2.3, 2.4, 2.5, 2.6:

- **Read:** ROADMAP, REQUIREMENTS, STATE; phases/{N}-CONTEXT, RESEARCH, *-PLAN; codebase; SUMMARYs, VERIFICATION.
- **Write:** CONTEXT (if discuss run), RESEARCH, *-PLAN, *-SUMMARY, VERIFICATION, UAT; STATE if updated.
...
| Step | Read | Write |
|------|------|--------|
| 1 | `.planning/ROADMAP.md`, `.planning/REQUIREMENTS.md` | — |
| 2 | `.planning/phases/{phase}/{phase}-VERIFICATION.md` for each phase in scope | — |
| 3 | Phase SUMMARYs (exports/APIs), requirement IDs | — (integration checker report in memory) |
...
| Step | Read | Write |
|------|------|--------|
| 1 | `.planning/PROJECT.md`, `.planning/STATE.md`, `.planning/ROADMAP.md` | — |
| 2 | — | `.planning/STATE.md` (update) |
| 3 | — | `.planning/research/*` (if research enabled) |
| 4 | RESEARCH, STATE | `.planning/REQUIREMENTS.md` (update), `.planning/ROADMAP.md` (new version), `.planning/STATE.md` |
...
| 2 | For each phase: `.planning/phases/{phase}/` (CONTEXT, *-PLAN, *-SUMMARY, VERIFICATION, UAT) | — |
...
- **pause-work:** Read ROADMAP, current phase dir; write STATE.md.
- **resume-work:** Read STATE.md only.
...
- **templates/** — project, requirements, roadmap, state, context, plan, summary, verification, uat (read when **writing** .planning files).
